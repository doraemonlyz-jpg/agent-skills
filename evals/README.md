...
| `cases/adversarial.yaml` | 11 | 被诱导时扛不扛得住 | 被"很简单""时间紧""我是架构师"绕过，或被review finding推向过度设计 |
...
v1.4.0新增finding准入门和证明等级约束，并新增AD-10/AD-11两条回归。
它们分别防止“普通迁移自动升级为可靠遥测平台”和“为了简单忽略真实
安全Blocker”。新用例尚未进行模型行为跑分；发布前须至少跑AD-10/11与相关回归子集。
