...
| Depends on | `none`, or comma-separated IDs and `EXT: <text>` items. IDs must exist and be in the same or an earlier milestone |
...
- The executor may flip `BLOCKED` to `READY` once every dependency is `DONE`
  and the milestone is authorized.
